...
Скрипт гарантирует соблюдение следующих ограничений:

- **Лимит правил**: Финальное количество правил всегда ≤ `max_entries` (по умолчанию 2000)
- **Полное покрытие**: Все целевые IP покрыты (если лимит правил достаточен)
- **Доступность /32 правил**: Все /32 правила для целевых IP всегда доступны как кандидаты

**Как это работает:**
- Алгоритм автоматически добавляет /32 правила для непокрытых IP после агрегации
- Если общее количество правил превысит лимит, агрегированные правила оптимизируются для максимального покрытия
- Жадный алгоритм использует итеративный подход для поиска оптимального баланса
